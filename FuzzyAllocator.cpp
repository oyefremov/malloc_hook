#include "FuzzyAllocator.h"

#include <csignal>
#include <cstddef>

void* fuzzy_allocator::malloc(size_t size, fuzzy_allocator::malloc_func* defaultMalloc) noexcept
{
...
void* fuzzy_allocator::realloc(void* ptr, size_t size, realloc_func* defaultRealloc) noexcept
{
    if (interceptAllocation(size)) {
        return nullptr;
    }
...
void* fuzzy_allocator::calloc(size_t num, size_t size, calloc_func* defaultCalloc) noexcept
{
...
void fuzzy_allocator::failAfter(size_t index) noexcept
{
    m_failIndex = index;
}

void fuzzy_allocator::abortAt(size_t index) noexcept
{
    m_breakIndex = index;
}
