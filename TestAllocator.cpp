...
void* test_allocator::malloc(size_t size, test_allocator::malloc_func* defaultMalloc) noexcept
{
...
void* test_allocator::realloc(void* ptr, size_t size, realloc_func* defaultRealloc) noexcept
{
...
void* test_allocator::calloc(size_t num, size_t size, calloc_func* defaultCalloc) noexcept
{
...
void test_allocator::free(void* ptr, test_allocator::free_func* systemFree) noexcept
{
    freeAllocation(ptr);
    systemFree(ptr);
}
...
void test_allocator::freeAllocation(void* ptr)
{
    if (ptr){
        auto pos = std::find_if(
            m_allocations.begin(), m_allocations.end(), [ptr](const AllocationInfo& info) {
                return info.ptr == ptr;
            });
...
void test_allocator::failAt(size_t index) noexcept
{
    m_failIndexe = index;
}

void test_allocator::abortAt(size_t index) noexcept
{
    m_breakIndexe = index;
}
...
const std::vector<test_allocator::AllocationInfo>& test_allocator::leaks() const noexcept
{
