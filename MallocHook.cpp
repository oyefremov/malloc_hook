...
void* malloc_hook::realloc(void* ptr, size_t size, realloc_func* defaultRealloc) noexcept
{
    return defaultRealloc(ptr, size);
}

void* malloc_hook::calloc(size_t num, size_t size, calloc_func* defaultCalloc) noexcept
{
...
bool malloc_hook::isEnabled() const
{
    return g_mallocHook == this;
}
...
{
    static auto* defaultMalloc = reinterpret_cast<decltype(&malloc)>(dlsym(RTLD_NEXT, "malloc"));
...
{
    static auto* defaultRealloc = reinterpret_cast<decltype(&realloc)>(dlsym(RTLD_NEXT, "realloc"));
...
{
    static auto* defaultCalloc = reinterpret_cast<decltype(&calloc)>(dlsym(RTLD_NEXT, "calloc"));
...
{
    static auto* defaultFree = reinterpret_cast<decltype(&free)>(dlsym(RTLD_NEXT, "free"));
    assert(defaultFree);
